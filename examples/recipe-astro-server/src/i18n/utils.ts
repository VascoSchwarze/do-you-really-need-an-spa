...
const statsPath: Record<Lang, string> = { de: "statistik", en: "stats" };

export function localizedPath(lang: Lang, page: "home" | "recipes" | "about" | "stats", slug?: string): string {
	if (page === "home") return `/${lang}/`;
	if (page === "about") return `/${lang}/${aboutPath[lang]}/`;
	if (page === "stats") return `/${lang}/${statsPath[lang]}/`;
	if (page === "recipes") {
